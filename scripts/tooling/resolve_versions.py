...
def github_release_tag(environment: Mapping[str, str]) -> str | None:
    direct_tag = environment.get("SUNCLUB_RELEASE_TAG")
    if direct_tag:
        return direct_tag
...
def resolve_marketing_version(
    ...
    explicit = environment.get("SUNCLUB_MARKETING_VERSION")
    if explicit:
        return explicit
...
def shell_exports(versions: ResolvedVersions) -> str:
    values = {
        "SUNCLUB_MARKETING_VERSION": versions.marketing_version,
        "SUNCLUB_BUILD_NUMBER": versions.build_number,
    }
    return "\n".join(
