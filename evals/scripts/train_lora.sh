...
#
# Prerequisites:
#   1. Run `just bootstrap` from this repo root.
#   2. Clone apple/ml-fastvlm and install its training extras (for example `uv pip install -e ".[train]"`).
#   3. Download Stage 3 PyTorch checkpoint: bash get_models.sh (from the FastVLM repo)
#   4. Build dataset: just collect-data
#
# Usage:
...
if [[ ! -f $TRAIN_JSON ]]; then
  echo "ERROR: $TRAIN_JSON not found. Run just collect-data first." >&2
  exit 1
fi
...
echo ""
echo "Next steps:"
echo "  1. Merge LoRA:  bash evals/scripts/merge_and_export.sh $FASTVLM_REPO $CHECKPOINT $OUTPUT_DIR"
echo "  2. Run eval:    just benchmark"
