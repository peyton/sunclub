...
- Bootstrap must remain lockfile-enforced and repo-local: `scripts/tooling/bootstrap.sh` runs `mise install --locked`, and the shared mise wrapper uses repo-local config/trust paths instead of `mise trust`. Tool-version PRs that change `mise.toml` are incomplete until `mise.lock` is deliberately refreshed with `mise lock` and committed; do not accept incidental lockfile drift.
