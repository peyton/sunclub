...
- Bootstrap must remain lockfile-enforced and repo-local: `scripts/tooling/bootstrap.sh` runs `mise install --locked`, and the shared mise wrapper uses repo-local config/trust paths instead of `mise trust`. Renovate tool-version PRs can merge green after changing only `mise.toml`; if Renovate comments that `mise.lock` artifact updates failed because `mise.toml` was not trusted, treat that as the same follow-up lock refresh after confirming CI is green and the PR is `mise.toml`-only. Do not add version-specific AGENTS notes for each tool-only bump. After one or several land, start from current `origin/master`, run `mise lock` from a clean follow-up branch so every pending tool bump is captured in one lockfile, and verify with `MISE_CONFIG_DIR="$PWD/.config/mise" MISE_TRUSTED_CONFIG_PATHS="$PWD" MISE_YES=1 mise install --locked --dry-run` before relying on locked bootstrap or release workflows. When the lockfile is known stale, avoid plain `mise exec` for doc-only checks because it can rewrite `mise.lock`. Do not accept incidental lockfile drift in unrelated worktrees.
...
- Cloudflare Pages deploy and rollback workflows both pin `cloudflare/wrangler-action`; keep `.github/workflows/deploy-web-cloudflare.yml` and `.github/workflows/rollback-web-cloudflare.yml` in sync when updating the action, and run the closest web/Cloudflare validation such as `just web-check` or `just cloudflare-check`.
