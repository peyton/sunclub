...
- Bootstrap must remain lockfile-enforced and repo-local: `scripts/tooling/bootstrap.sh` runs `mise install --locked`, and the shared mise wrapper uses repo-local config/trust paths instead of `mise trust`. Renovate tool-version PRs can merge green after changing only `mise.toml`; after one lands, run `mise lock` from a clean follow-up branch and commit the resulting `mise.lock` before relying on locked bootstrap or release workflows. Do not accept incidental lockfile drift in unrelated worktrees.
