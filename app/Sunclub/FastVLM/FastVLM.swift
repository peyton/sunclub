...
public class FastVLMProcessor: UserInputProcessor {
    ...
    public func prepare(prompt: UserInput.Prompt, imageTHW: THW?) -> String {
        ...
        switch prompt {
        case .text(let text):
            messages = DefaultMessageGenerator().generate(messages: [.user(text)])
        case .messages(let rawMessages):
            messages = rawMessages
        case .chat(let chat):
            messages = DefaultMessageGenerator().generate(messages: chat)
        }
        ...
        if let imageTHW {
            ...
            lastMessage += String(repeating: config.imageToken, count: numImageTokens)
        }
