...
- `scripts/appstore/capture-screenshots.sh` builds the real app, boots the simulator named in the manifest, captures raw route screenshots through `UITEST_MODE`, then renders marketing-composed App Store pages with current screenshots inserted into device frames.
