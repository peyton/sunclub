...
`just test-ui` did not complete because the simulator repeatedly failed to launch the UI test runner with `FBSOpenApplicationServiceErrorDomain` / `Application info provider ... returned nil`.
...
The iOS app lives under `app/Sunclub`. Private iCloud history sync is separate and lives in `CloudSyncCoordinator`; this plan must not disable private iCloud history sync.

Apple’s App Privacy definition treats data as collected when it is transmitted off-device in a way that the developer or partners can access for longer than real-time request servicing. Data processed only on-device, private user iCloud storage that the developer cannot access, and local files do not need to be declared as collected by the developer for this release posture.
