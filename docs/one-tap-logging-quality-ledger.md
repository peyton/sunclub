...
- [x] Q010 | Evidence: Control Center actions run the same App Intent as Shortcuts. | Fix: `LogSunscreenIntent` still delegates to `.logToday`, which now uses the resolver. | Verification: automation runtime tests cover the App Intent call boundary.
...
- [x] Q050 | Evidence: App Intent discoverability must remain intact. | Fix: `LogSunscreenIntent` title and shortcut catalog unchanged. | Verification: App Intent code unchanged except delegated runtime behavior.
...
- [x] Q057 | Evidence: stale widget timeline state should not determine write defaults. | Fix: default resolution reads current storage at tap time. | Verification: runtime/deep-link tests seed storage immediately before action.
...
- [x] Q085 | Evidence: Browser advisory flagged extension cache staleness. | Fix: action handlers resolve from current model/widget stores at execution. | Verification: seed-immediately-before-action tests.
