...
- [x] (2026-08-11) Disabled the production public CloudKit transport while Activity sharing remains absent from the visible production route set, and aligned the privacy manifest, App Store metadata, and public privacy copy with the disabled transport.
...
- Observation: Later product simplification kept compatibility routes but removed Activity sharing from the visible production experience, while the public transport and data-collection declarations remained enabled.
  Evidence: production routes normalize social destinations to Settings, but the production flavor still set `publicAccountabilityTransportEnabled: true` and the privacy manifest still declared public-sharing data collection before the 2026-08-11 trust pass.
...
- Decision: Supersede the 2026-05-15 production decision by disabling direct public transport until Activity sharing is intentionally restored as a visible feature.
  Rationale: Dormant developer-accessible transport should not remain enabled when people cannot discover or manage the feature. Compatibility code and local state remain so a future deliberate release can restore the feature with matching UI, tests, and privacy declarations.
...
- The direct-accountability implementation remains available behind its runtime capability, but production builds now use the no-op transport while the feature is hidden. Re-enabling it requires a visible product route and matching App Privacy disclosures in the same release.
